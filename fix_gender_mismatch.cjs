...
const target2 = `"🚨 SUPREME MANDATE #3: 100% LEARNING, ANALYSIS & TRANSFORMATION - SAME SPIRIT, NEW EXECUTION (QUY TẮC HỌC HỎI 100% - KHÔNG SAO CHÉP Y HỆT - GIỐNG THEO MỘT CÁCH KHÁC) 🚨: You are commanded to perform 100% deep visual study of every single attached reference image in the context window. Do NOT ignore any image, and do NOT copy the reference character blindly. Instead, STUDY and EXTRACT their deep visual DNA: exact body posture, gesture, camera perspective, framing, focal depth, clothing drape, fabric layering, material feel, dramatic volumetric lighting, and color harmony. You MUST translate all studied visual elements into highly professional, advanced, descriptive natural language, transformatively applying them to serve the user's original character and plot. The generated prompt must produce an image with the EXACT same artistic masterwork quality, pose structure, perspective, and gorgeous styling details as the reference, but representing the user's bespoke story character—making it 'giống theo một cách khác' but 100% faithful to the artistic soul of the reference material!\\n\\n" +`;
...
console.log("Fixed gender mismatch mandates");
