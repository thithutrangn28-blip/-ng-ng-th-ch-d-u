...
const startIdx = code.indexOf('<ErrorBoundary>');
const endIdx = code.indexOf('</ErrorBoundary>') + '</ErrorBoundary>'.length;
...
const newBlock = `
            ...
            <section className="glass-panel">
              ...
              <div className="room-grid">
                {state.stories.map((s, idx) => {
                  ...
                  const cover = s.background || PRESET_BACKGROUNDS[idx % PRESET_BACKGROUNDS.length];
                  return (
                    <article className="profile-card" key={"story_" + s.id} style={{ cursor: 'pointer' }} onClick={() => enterStory(s.id)}>
                    ...
                    <article className="profile-card" key={\`main_room_\${r.id}_\${idx}\`} style={{ cursor: 'pointer' }} onClick={() => handleEnterRoom(r.id)}>
                      ...
                      <div className="profile-body">
                        ...
                        <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
                          <button className="btn primary small" style={{ flex: 1 }} onClick={(e) => { e.stopPropagation(); handleEnterRoom(r.id); }}>♥ vào phòng</button>
                          {isCustom && (
                            <>
        ...
        {currentView === "room" && currentRoomId && roomDef && currentRoomState && (
          <RoomView 
            ...
            save={save}
            toast={toast}
            onBack={() => { setRoomProgress(0); setCurrentView("drawer"); setCurrentRoomId(null); }}
