...
# The user wants to update coreMandates and the general logic.
# Wait, let's actually just replace from 'let coreMandates = `' to '`;\n          finalSystemPrompt ='

replacement = r'''let coreMandates = `[SYSTEM INSTRUCTIONS — ART INTELLIGENCE LAYER]
...
# We want to replace from 'let coreMandates = `### 🚨 MỆNH LỆNH THỐNG TRỊ' up to '5. **STRICT CLEAN OUTPUT**.*?\`;'
# Let's use regex
pattern = re.compile(r'let coreMandates = `### 🚨 MỆNH LỆNH THỐNG TRỊ: HỆ THỐNG DỮ LIỆU CỐT LÕI \(CANONICAL TRUTH\) 🚨.*?mô tả nghệ thuật\.\n`;', re.DOTALL)

if not pattern.search(content):
    print("Pattern not found!")
    # Let's try an alternative match
    pattern2 = re.compile(r'let coreMandates = `.*?mô tả nghệ thuật\.\n`;', re.DOTALL)
